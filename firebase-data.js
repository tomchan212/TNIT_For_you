...
import { ADMIN_EMAIL, FCM_VAPID_KEY, firebaseConfig, participantEmail, participantEmails } from './firebase-config.js?v=8701a69';
 ...
 * like "A1" are shorter, so we repeat the id until it meets the minimum.
...
export async function renameParticipantId(oldId, newId, { groupId, displayName, password } = {}) {
  ...
  if (!isSeatParticipantId(to)) throw new Error('新編號格式不正確（例如 A1）');
